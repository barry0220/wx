        ...
        <div id="ListFilesContainer" data-load="true" style="overflow: hidden; margin: 0px 60px; border-left: 1px solid rgb(227, 227, 227); border-right: 1px solid rgb(227, 227, 227); display: block;">
            <table width="60%">
                ...
                <?php if(is_array($infos)): foreach($infos as $key=>$v): ?><tr onclick="changebg(this)">
                        ...
                        <td>
                            <input type="hidden" value="<?php echo ($v['linkpath']); ?>" >
                            <a href="javascript:void(0);" ondblclick="dbhref(this)"><?php echo ($v['filename']); ?></a>
...
<script>
    ...
    function getPath(){
        ...
        for (var i=0;i<len;i++){
            ...
            str += '<td style="white-space:nowrap;"> ' +
                ...
                '<a href="/list/index?n='+tagpath+'">'+paths[i]+'</a> ' +
                '<span id="crumb2" class=""><span class="crumbarrowico"></span></span> ' +
                '</td>';
        }
        tr.append(str);
    ...
    function dbhref(obj){
        ...
        if(type == 'a'){
            location.href="/list/index?n="+linkpath;
        }else{  //如果是文件类型  进行下载
            location.href = "/list/downloadFile?linkpath="+linkpath+"&filename="+filename;
        }
    }
