...
class Wxapi{
    private $app_id;
    private $app_secret;
    private $app_mchid; //商户号id
    public function __construct() {
        $this->app_id = APPID;
        $this->app_secret = APPSECRET;
        $this->app_mchid = "1348867301";
    }
     ...
     */
    public function pay($re_openid,$worth,$wishing='邀好友再赚红包',$act_name='艾优云12.12纹眉活动',$send_name="艾飞思科技"){
        include_once('wxpay/WxHongBaoHelper.php');
        $commonUtil = new CommonUtil();
        $wxHongBaoHelper = new WxHongBaoHelper();
        $wxHongBaoHelper->setParameter("nonce_str", $this->great_rand());//随机字符串，不能长于 32 位
        ...
        $wxHongBaoHelper->setParameter("nick_name", '艾飞思');//提供方名称
        $wxHongBaoHelper->setParameter("send_name", $send_name);//红包发送者名称
        $wxHongBaoHelper->setParameter("re_openid", $re_openid);//openid
        $wxHongBaoHelper->setParameter("total_amount", $worth);//付款金额，单位分
        $wxHongBaoHelper->setParameter("min_value", $worth);//最小红包金额，单位分
        $wxHongBaoHelper->setParameter("max_value", $worth);//最大红包金额，单位分
        $wxHongBaoHelper->setParameter("total_num", 1);//红包収放总人数
        $wxHongBaoHelper->setParameter("wishing", $wishing);//红包祝福
        $wxHongBaoHelper->setParameter("client_ip", '42.96.249.245');//调用接口的机器 Ip 地址
